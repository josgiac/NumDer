function [d,ifail] = NumDerEquispacedData(a,b,o,f)
% [d,ifail] = NumDerEquispacedData(a,b,o,f)
% INPUT:
% [a,b] - domanin of the function F
% o     - order of the computed derivative of F, o=1,2,...
%         f(1:n+1) - vector containing the values of F at n+1 uniformly distributed points
%         in [a,b] ---> f(j)=F(a+(j-1)(b-a)/n);
% OUTPUT:
% d(1:m)- derivative of order o of F, m=n-o+1, m must be >=3; 
%           d(j)=derivative of order o of F at x=a+(j-1+o/2)(b-a)/n;
...
d=dct(Fg,'Type',4)/(b-a);
for p=2:o %computation of derivatives > 1
    m=n-p+1;
    na=a+hTrue*(p-1)/2;
    nb=a+hTrue*(m+(p-1)/2);
