...
% NumDerEquispacedData computes the derivatives of a function F starting from its values at uniformly distributed points
%
% INPUT:
...
% Check for error indicators and warnings
ifail=0;
if (a>=b || nu<1)
    ifail=1;
elseif n< nu+2
...
% Computation of derivatives of order p> 1, p<=nu
for p=2:nu 
    m=n-p+1;
    na=a+hTrue*(p-1)/2;
    nb=a+hTrue*(m+(p-1)/2);
